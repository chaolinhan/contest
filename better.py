...
# ==========================================
# 1b. Incremental overlap primitives
# ==========================================
@njit(fastmath=True)
                ...
                if na_mode == 1:
                        ...
                        if ax == 0:
                            ...
                            if zx_idx != -1:
                                ...
                                if abs(coeff) > 1e-5 and abs(median_v - cv) > 1e-3:
                                    raw_delta = ((median_v - cv) / coeff) * next_rand(rng_state) * 1.2
                                    delta = max(min(raw_delta, base_step * 2.0), -base_step * 2.0)
                                    is_net_aware = True
                        else:
                    ...
                    if next_rand(rng_state) < 0.5 and len_x > 0:
                        ...
                        if zx_idx != -1:
                            ...
                            coeff = M_x[target_b, zx_idx]
                            if abs(coeff) > 1e-5:
                                raw_delta = (dist / coeff) * next_rand(rng_state) * 1.5
                                delta = max(min(raw_delta, base_step * 2.0), -base_step * 2.0)
                                is_net_aware = True
                    elif len_y > 0:
                        zy_idx = box_to_zy[target_b]
...
class Solution:
    TIME_BUDGET = 119.0
    SEED = 246813579
    USE_INCR_OVERLAP = True
    RESTART_MULT = 2.0
    LARGE_N_MULT = 2.0
    NET_AWARE_MODE = 0
    FEAS_BACKSTOP = True
    # --- per-tier dispatch (taxonomy-guided; see cases_taxonomy.md) ---
    # Master switch. The table reproduces cur_181's dispatch as the baseline
    # (every tier -> 1.0); the single measured refinement targets case9
    # large-scale. cur_181 gives case9 ~31 restarts; the SA reaches its basin
    # in fewer, so 30+ over-shortens slices. div=0.75 maps 31 -> ~23 restarts
    # (solution_p2's measured sweet spot ~24, LARGE_N_MULT=4).
    #   * LARGE-scale case9 (case9_5-_9): -5.5% cost, consistent (the hard-to-
    #     pack, restart-limited regime the taxonomy flags). KEEP.
    #   * SMALL-scale case9 (case9_0-_4): mixed/neutral (case9_0 +2.7%),
    #     so it is left at baseline -- box-scale tiering targets only the
    #     regime that actually benefits.
    # scale_div divides restart_time_target: <1 => fewer, deeper restarts.
    # Set USE_TIER_DISPATCH False to recover exact cur_181.
    USE_TIER_DISPATCH = True
    SCALE_AREA_THRESH = 1.5e11  # scaled avg box-area (== 1500 original units) splitting sm/lg
    # tier key f"{band}_{scale}", band in {S,A,B,C} by N, scale in {sm,lg} by
    # avg box area. Value = restart_scale_div (1.0 == cur_181 baseline).
    TIER_DISPATCH = {
        "S_lg": 0.75,  # case9 large-scale (case9_5-_9): 31 -> 23 restarts (-5.5% measured)
    }

    def _tier_key(self, N, total_area):
        """Classify a case into a (band, scale) tier key for dispatch.
        band mirrors the difficulty taxonomy (S>A>B>C by N); scale splits the
        within-group box-size axis (_0-_4 small vs _5-_9 large, ~6-7x area) --
        the within-group difficulty driver that cur_181's constraint_ratio-only
        dispatch does not see."""
        if N > 60:
            band = "S"
        elif N > 40:
            band = "A"
            ...
            band = "C"
        avg_box_area_scaled = total_area / max(1, N)
        scale = "lg" if avg_box_area_scaled > self.SCALE_AREA_THRESH else "sm"
        return f"{band}_{scale}"

    def _restart_scale_div(self, N, total_area):
        """Per-tier divisor on restart_time_target (>1 => more restarts).
        Defaults to 1.0 (== cur_181 baseline); TIER_DISPATCH overrides per tier."""
        if not self.USE_TIER_DISPATCH:
            return 1.0
        return self.TIER_DISPATCH.get(self._tier_key(N, total_area), 1.0)
    ...
    def solve(self, data):
        ...
        nets = data.get("nets", [])
        num_nets_total = len(nets)
        if nets:
            max_len = max(len(n) for n in nets)
            net_mat = np.full((num_nets_total, max_len), -1, dtype=np.int32)
        ...
        total_area = np.sum(box_sizes_float[:, 0] * box_sizes_float[:, 1]) if N > 0 else 0.0
        
        # 1. 动态边界范围：保障不紧贴也不无限散列
        scatter_range = max(400000.0, np.sqrt(total_area) * 2.0)
        
        # 2. 计算特征：网络密度、自由变量约束比
        net_density = num_nets_total / float(max(1, N))
        ...
        phase1_time_ratio = 0.60 if constraint_ratio < 0.5 else 0.45
        
        # 4. 动态步数调整：复杂情况给足时间寻优
        inner_steps = int(max(200, min(1000, (num_free_x + num_free_y) * 6)))
        ...
        # per-tier, box-scale-aware refinement (taxonomy: large-scale large-N is
        # the worst-packed, restart-limited regime). No-op (div=1.0) unless the
        # tier is listed in TIER_DISPATCH.
        restart_time_target = restart_time_target / self._restart_scale_div(N, total_area)
        ...
        for r in range(n_restarts):
            ...
            p1_duration = current_restart_budget * phase1_time_ratio
            p2_duration = current_restart_budget * 0.35
            tune_duration = current_restart_budget * 0.10

            rng = np.array([seeds[r]], dtype=np.uint32)
            cur_Zx = np.zeros(num_free_x, dtype=np.float64)
            cur_Zy = np.zeros(num_free_y, dtype=np.float64)
            for i in range(num_free_x): cur_Zx[i] = (next_rand(rng) - 0.5) * scatter_range
            for i in range(num_free_y): cur_Zy[i] = (next_rand(rng) - 0.5) * scatter_range
            ...
            while time.time() < p1_deadline:
                ...
                cur_Zx, cur_Zy, bZx, bZy, bc, bov = run_sa_steps_float_incr(
                    ...
                    v2b_x_idx, v2b_x_val, v2b_y_idx, v2b_y_val, box_to_nets_mat, box_to_zx, box_to_zy,
                    scratch_x2, scratch_y2, scratch_ix1, scratch_ix2, scratch_iy1, scratch_iy2, net_moved, in_box,
                    1, T, step_size, rng, p1_w_area, p1_w_hpwl, inner_steps, na_prob, na_mode, na_scr)
            ...
            while time.time() < p2_deadline:
                ...
                cur_Zx, cur_Zy, bZx, bZy, bc, bov = run_sa_steps_float_incr(
                    ...
                    v2b_x_idx, v2b_x_val, v2b_y_idx, v2b_y_val, box_to_nets_mat, box_to_zx, box_to_zy,
                    scratch_x2, scratch_y2, scratch_ix1, scratch_ix2, scratch_iy1, scratch_iy2, net_moved, in_box,
                    2, T, step_size, rng, p2_w_area, p2_w_hpwl, inner_steps, na_prob, na_mode, na_scr)
            ...
            while time.time() < tune_deadline:
                ...
                X_int, Y_int, bX, bY, bc, bov = run_integer_tuning_steps_incr(
                    ...
                    b2b_x_idx, b2b_x_val, b2b_y_idx, b2b_y_val, box_to_nets_mat,
                    scratch_x2, scratch_y2, scratch_ix1, scratch_ix2, scratch_iy1, scratch_iy2, net_moved, in_box,
                    rng, p2_w_area, p2_w_hpwl, T_tune, inner_steps)
